import { getHistoricalTransactions } from '../services/etherscanService';
import { getLastProcessedBlock, updateLastProcessedBlock } from '../services/stateService';
...
export const runBatchJob = async () => {
  const endBlock = 99999999; // Adjust range for how much historical data to index
  const pageSize = 100; // Fetch 100 transactions per request
  ...
  try {
    ...
    while (hasMoreData) {
      console.log(`Fetching page ${page} of historical transactions from block ${lastProcessedBlock}...`);
      ...
      const highestBlock = Math.max(...transactions.map((tx: { blockNumber: any; }) => Number(tx.blockNumber)));
